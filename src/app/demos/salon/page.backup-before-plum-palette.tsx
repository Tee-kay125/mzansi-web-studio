...
function SalonImage({
  ...
  return (
    <div className={`group relative overflow-hidden bg-gradient-to-br from-[#f7d7c5] via-[#f8eee7] to-[#d6a86d] ${className}`}>
      ...
      />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-[#0f1115]/55 via-transparent to-transparent" />
      <div className="absolute bottom-4 left-4 right-4 rounded-2xl bg-white/92 px-4 py-3 shadow-xl backdrop-blur">
        <p className="text-xs font-black uppercase tracking-[0.22em] text-[#7a5a24]">{label}</p>
      </div>
    </div>
  );
}
...
export default function SalonDemoPage() {
  return (
    <main className="bg-[#faf7f0] text-[#111111]">
      <a
        href="#book"
      ...
      <section className="relative overflow-hidden bg-[#0f1115] text-white">
        ...
        <div className="absolute inset-0 bg-[#0f1115]/18" />
        <div className="absolute inset-0 bg-gradient-to-r from-[#0f1115]/78 via-[#0f1115]/36 to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-b from-transparent via-transparent to-[#0f1115]/38" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_28%,rgba(16,185,129,0.12),transparent_30%),radial-gradient(circle_at_16%_78%,rgba(201,164,92,0.10),transparent_34%)]" />
          ...
          <div className="grid gap-12 pb-16 pt-8 md:grid-cols-[0.95fr_1.05fr] md:items-center md:pb-24 md:pt-12">
            <div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                <a
                  href="#book"
                  className="inline-flex items-center justify-center rounded-2xl bg-[#e8c77a] px-6 py-4 font-black text-[#111111] shadow-lg shadow-amber-950/25 transition hover:bg-[#e8c77a]"
                >
              ...
              <div className="mt-10 grid gap-4 sm:grid-cols-3">
                {[
                  ...
                  <div key={label} className="rounded-3xl border border-white/12 bg-white/12 p-5 backdrop-blur">
                    <p className="text-3xl font-black text-[#e8c77a]">{value}</p>
            ...
            <div className="relative">
              <div className="absolute -left-8 -top-8 hidden h-32 w-32 rounded-full bg-[#c9a45c]/25 blur-3xl md:block" />
              ...
              <div className="relative rounded-[2.4rem] border border-white/18 bg-white/12 p-3 shadow-2xl shadow-black/35 backdrop-blur">
                ...
                <div className="absolute bottom-8 left-8 right-8 rounded-3xl bg-white/95 p-5 text-[#0f1115] shadow-2xl backdrop-blur">
                  <div className="flex items-center gap-4">
                    <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#f7efe0] text-[#8b6a2f]">
      ...
      {/* BRAND STRIP */}
      <section className="border-b border-[#dfcda8] bg-white px-4 py-8 md:px-6">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">Bella Hair Studio</p>
          ...
          <div className="flex flex-wrap gap-3 text-sm font-bold text-stone-600">
            {['Braids', 'Treatments', 'Silk Press', 'Wig Installation'].map((item) => (
              <span key={item} className="rounded-full bg-[#f7efe0] px-4 py-2">{item}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      {/* SPECIAL OFFER */}
      <section className="bg-[#f7efe0] px-4 py-12 md:px-6">
        <div className="mx-auto grid max-w-7xl gap-6 rounded-[2rem] border border-[#c9a45c] bg-white p-6 shadow-sm md:grid-cols-[1fr_auto] md:items-center md:p-8">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">New client special</p>
          ...
          <a href="#book" className="inline-flex items-center justify-center rounded-2xl bg-[#0f1115] px-6 py-4 font-black text-white">
      ...
      <section id="services" className="px-4 py-20 md:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">Signature services</p>
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {services.map((service) => (
              <div key={service.title} className="group overflow-hidden rounded-[2rem] border border-[#dfcda8] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
                <SalonImage src={service.image} alt={service.title} label={service.title} className="h-64" />
                <div className="p-6">
                  <h3 className="text-xl font-black">{service.title}</h3>
                  <p className="mt-3 text-2xl font-black text-[#8b6a2f]">{service.price}</p>
      ...
      <section className="bg-white px-4 py-20 md:px-6">
        <div className="mx-auto grid max-w-7xl gap-12 md:grid-cols-2 md:items-center">
          ...
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">The experience</p>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {reasons.map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl bg-[#f7efe0] p-4 font-bold text-stone-700">
                  <CheckCircle2 className="h-5 w-5 text-[#8b6a2f]" /> {item}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* PRICING MENU */}
      <section className="bg-[#faf7f0] px-4 py-20 md:px-6">
        <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[0.8fr_1.2fr] md:items-start">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">Price menu</p>
          ...
          <div className="rounded-[2rem] border border-[#dfcda8] bg-white p-6 shadow-sm md:p-8">
            <div className="grid gap-4 sm:grid-cols-2">
              {priceList.map(([name, price]) => (
                <div key={name} className="flex items-center justify-between rounded-2xl bg-[#f7efe0] p-4">
                  <span className="font-black">{name}</span>
                  <span className="font-black text-[#8b6a2f]">{price}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* WHY CHOOSE */}
      <section className="bg-[#0f1115] px-4 py-20 text-white md:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#e8c77a]">Why choose us</p>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {[
              ...
              <div key={item.title} className="rounded-[2rem] border border-white/10 bg-white/10 p-6 backdrop-blur">
                <item.icon className="mb-6 h-8 w-8 text-[#e8c77a]" />
      ...
      <section className="bg-white px-4 py-20 md:px-6">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">Testimonials</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">What clients say.</h2>
          </div>

          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {testimonials.map((testimonial) => (
              <div key={testimonial.name} className="rounded-[2rem] border border-[#dfcda8] bg-[#faf7f0] p-6 shadow-sm">
                <Quote className="mb-5 h-8 w-8 text-[#8b6a2f]" />
      ...
      <section className="bg-[#faf7f0] px-4 py-20 md:px-6">
        <div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-[0.8fr_1.2fr]">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">Questions</p>
            <h2 className="mt-3 text-4xl font-black tracking-tight md:text-5xl">Make booking feel easy.</h2>
          </div>
          <div className="space-y-4">
            {faqs.map((faq) => (
              <div key={faq.q} className="rounded-[1.5rem] border border-[#dfcda8] bg-white p-6 shadow-sm">
      ...
      <section id="book" className="px-4 py-20 md:px-6">
        <div className="mx-auto grid max-w-7xl gap-8 md:grid-cols-[0.95fr_1.05fr]">
          <div className="rounded-[2rem] bg-[#050505] p-8 text-white shadow-2xl md:p-10">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#e8c77a]">Book now</p>
            ...
            <div className="mt-8 space-y-4">
              ...
              <div className="flex items-center gap-4 rounded-2xl bg-white/10 p-4 font-semibold text-white"><Phone className="h-5 w-5 text-[#e8c77a]" /> 063 000 0000</div>
              <div className="flex items-center gap-4 rounded-2xl bg-white/10 p-4 font-semibold text-white"><AtSign className="h-5 w-5 text-[#e8c77a]" /> @bellahairstudio</div>
              <div className="flex items-center gap-4 rounded-2xl bg-white/10 p-4 font-semibold text-white"><MapPin className="h-5 w-5 text-[#e8c77a]" /> Cape Town, South Africa</div>
            </div>
          </div>

          <div className="rounded-[2rem] border border-[#dfcda8] bg-white p-8 shadow-sm md:p-10">
            <p className="text-sm font-black uppercase tracking-[0.25em] text-[#8b6a2f]">Opening hours</p>
            <h3 className="mt-3 text-3xl font-black">Visit the salon</h3>
            <div className="mt-8 space-y-4">
              {hours.map(([day, time]) => (
                <div key={day} className="flex items-center justify-between rounded-2xl bg-[#f7efe0] p-4">
                  <div className="flex items-center gap-3"><Clock className="h-5 w-5 text-[#8b6a2f]" /><p className="font-black">{day}</p></div>
                  <p className="font-semibold text-stone-600">{time}</p>
                </div>
              ))}
            </div>

            <div className="mt-8 flex h-64 items-center justify-center rounded-[1.5rem] bg-gradient-to-br from-[#d8c29a] to-[#f4ead8] text-center">
              <div><MapPin className="mx-auto mb-3 h-8 w-8 text-[#8b6a2f]" /><p className="font-black text-stone-800">Google Maps Placeholder</p><p className="mt-2 text-sm text-stone-600">A real map can be embedded here.</p></div>
            </div>
          </div>
        </div>
      </section>
